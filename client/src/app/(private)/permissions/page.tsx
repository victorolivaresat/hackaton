...
const Permissions = () => {
  ...
  const averagePermissionsPerModule =
    modules.length > 0 ? (permissions.length / modules.length).toFixed(1) : "0";
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.entries(modulePermissionCount).map(
                ([moduleId, count]) => (
                  <div
                    key={moduleId}
      ...
      {loading ? (
        ...
        <DataTable
          columns={columns({
            ...
            onPermissionDeleted: fetchPermissions,
          })}
          data={permissions}
          headerActions={
            <PermissionCreateButton onPermissionCreated={fetchPermissions} />
          }
        />
      )}
    </div>
